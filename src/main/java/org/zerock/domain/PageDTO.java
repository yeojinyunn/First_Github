...
public class PageDTO {
	...
	private Criteria cri; // 페이지번호(pageNum)와 수량(amount)을 하나의 객체로 표현한 cri 필드 선언
	
	public PageDTO(Criteria cri, int total) {
		this.total = total;
		this.cri = cri;
		
		// 페이징의 끝 번호 (endPage) 계산식 적용
		// 여기서는 페이지 번호가 10개씩 보인다고 가정합니다.
		
		this.endPage = (int) (Math.ceil(cri.getPageNum() / 8.0)) * 8; 
		
		// 만약 화면에 10개씨기 글을 보여준다면, 시작 번호 (startPage)는 
		// 끝번호(endPage)에서 9를 뺀 값이 됩니다.
